    ...
    <div className="lg:col-span-6 flex flex-col space-y-2">
      <div className="flex items-center justify-between px-1">
        <span className="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
          ...
          {t('documents.originalArchivalScan', { defaultValue: 'Original Archival Scan' })}
        </span>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {isImage && !hasError && (
            <>
              ...
              <button
                ...
                className="p-1 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs transition-colors"
                title={t('documents.resetZoom', { defaultValue: 'Reset Zoom' })}
              >
          ...
          {fileUrl && (
            <a
              ...
              title={t('documents.openFullResolution', {
                defaultValue: 'Open full resolution file in new tab',
              })}
            >
      ...
      <div className="rounded-xl border border-slate-300 bg-slate-900/95 min-h-[520px] lg:h-[calc(90vh-130px)] overflow-auto p-3 flex items-center justify-center relative shadow-inner">
        ...
        {isLoading && !hasError && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/70 backdrop-blur-xs z-10 text-slate-300 gap-2">
            ...
            <span className="text-xs font-medium">
              {t('documents.loadingDocumentScan', { defaultValue: 'Loading document scan...' })}
            </span>
          </div>
        )}
        ...
        {hasError ? (
          <div className="flex flex-col items-center justify-center p-6 text-center text-slate-300 max-w-sm space-y-3">
            ...
            <div>
              <h4 className="font-semibold text-sm text-slate-100">
                {t('documents.scanFileNotFound', { defaultValue: 'Document Scan File Not Found' })}
              </h4>
              <p className="text-xs text-slate-400 mt-1">
                {t('documents.scanFileNotFoundDesc', {
                  defaultValue:
            ...
            <div className="flex items-center gap-2 pt-1">
              ...
              >
                ...
                {t('documents.retryLoading', { defaultValue: 'Retry Loading' })}
              </button>
              {fileUrl && (
                <a
                  href={fileUrl}
                ...
                >
                  <ExternalLink className="w-3.5 h-3.5" />
                  {t('documents.directLink', { defaultValue: 'Direct Link' })}
                </a>
              )}
            </div>
          </div>
          ...
          <iframe
            ...
            title={t('documents.archivalScanPdfTitle', {
              defaultValue: 'Archival Document Scan PDF',
            })}
            onLoad={() => setIsLoading(false)}
          />
        )}
      ...
      <div className="text-[10px] text-slate-400 text-right pr-1">
        {t('documents.fileLabel', { defaultValue: 'File' })}: {doc.fileName}
      </div>
    </div>
  );
};
